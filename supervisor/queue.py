...
def build_evolution_plan_task_text(
    ...
    return (
        ...
        "Passing cwd='/home/newnestor/Ouroboros/data/' or similar data paths will be REJECTED. "
        "Use relative paths from the repo root instead.\n"
        "- Time budget: reserve at least 1/3 of your tool budget for the commit step. "
        "Tasks that run out of tool budget before the repo_commit call are counted as FAILED.\n"
        "EXECUTION MODEL:\n"
        "- Phase 1 (diagnose): inspect the subsystem, identify the root cause.\n"
        "- Phase 2 (plan): write the plan with exact file paths and minimal changes.\n"
        "- Phase 3 (implement): make the changes, verify them, then repo_commit.\n"
        "- Phase 4 (confirm): report the concrete improvement in your final message.\n"
        "Do NOT skip Phase 3. Planning without implementation is not evolution.\n"
        "INSTRUCTIONS:\n"
        "1. Execute Phases 1-3: diagnose, plan, implement the fix.\n"
        "2. End with these headings exactly:\n"
        "PLAN_SUMMARY:\n"
        "TARGET_FILES:\n"
        "VALIDATION:\n"
        "3. repo_commit before the hard timeout fires.\n"
    )
